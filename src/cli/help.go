...
var cmdDocs = map[string][]string{
	...
	"prefop":        []string{"qshell [-d] prefop <PersistentId> <host>", "Query the fop status"},
	...
	"mpsaveas":      []string{"qshell saveas <sn> <wm> <ss> <pic bucket> <pic key> <video bucket> <video key> <http://host:port/bucket/key>", "Create a resource access url with fop and mpsaveas"},
